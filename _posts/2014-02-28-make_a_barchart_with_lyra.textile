...
Just a few days ago "Arvind Satyanarayan":https://twitter.com/arvindsatya1 from the "UW Interactive Data Lab":http://idl.cs.washington.edu/ released an alpha version of "Lyra":http://idl.cs.washington.edu/projects/lyra/ which promises to be a way to create complex data visualizations without code. 

A sort of open source Adobe Illustrator geared toward data visualizations.

There are a number of very interesting examples to learn from on the Lyra main page. These recreate some well known complex visuals. 

I wanted to see what would be required in creating a really basic barchart - as a way to start to learn more about this interesting tool. 

Here's the final product:

Pretty simple right? But "sometimes simple ways are the best":http://bost.ocks.org/mike/bar/ . So follow along with me for a bit of Lyra fun!

<div class="box">
*WARNING:* Lyra is labelled as _alpha_ and while already a great tool, its got some issues. A few times I had to restart from scratch after getting the tool into an state I couldn't recover from. I expect these issues to go away over time. 
</div>

h2. Your Visualization Development Environment

Here's what it looks like when you first "open Lyra":http://idl.cs.washington.edu/projects/lyra/app/

<div class="center">
<img class="center" src="http://vallandingham.me/images/lyra/lyra_01_small.png" alt="" style="border:1px dotted #cccccc;"/>
</div>

I've annotated the app to provide a bit of orientation.

<div class="center">
<img class="center" src="http://vallandingham.me/images/lyra/lyra_01_2_small.png" alt="" style="border:1px dotted #cccccc;"/>
</div>

So the far left area provides capabilities to add data to the visualization and manipulate this data. 
...
We will start with the data input and manipulation panel first.

h2. A Simple Data Pipeline

Let's pull in some data. To avoid complications, I used *medals*, one of the sample data sets provided by Lyra. This gives data about who won what at the Olympics. 
...
The original data includes gold, silver, and bronze medal winnings. Let's just go for golds. 
...
Then click on the _Filter_ button

<div class="center">
<img class="center" src="http://vallandingham.me/images/lyra/lyra_04_1.png" alt="" style="border:1px dotted #cccccc;"/>
</div>

We get a little Filter input field. We are going to perform our first data binding in Lyra now. This involves dragging column names from the Data section to a little circle _data binding_ visual.
...
This appears to be JavaScript, so your going to want to use the double equals: @Medal_Type == "Gold"@.

<div class="center">
<img class="center" src="http://vallandingham.me/images/lyra/lyra_07.png" alt="" style="border:1px dotted #cccccc;"/>
</div>

Once you have the filter setup, click _Add to Pipeline_. This will add the filter step to your pipeline (as you might guess by the name of the button).
...
If you look at the values shown in the Data panel, you should see that only those with @Medal_Type@ as _Gold_ are shown. Pretty cool right?

h3. Sort those Medals

Now I want to sort the data in order of medal counts, so the countries with the most appear at the top. You might have guessed it by now, but we are going to use the sort Transform.

Click on the _New Transform_ button again, put this time select _Sort_.
...
Then select _Descending_ for the _Order_. Once you add this to the pipeline, we should have all our data transformations done, and be ready for some visualizing. 
...
We see _Visualization_ at the top, and _Layer1_ which is clickable to show more options.

h3. Give Yourself Some Room
...
As you can see in the image above, I gave my visualization some space by changing the _Width_ to @700@ and _Height_ to @500@. I am assuming this is in pixels. For now, let's leave the _Padding_ setting alone.
...
Not a bad start. Lets move them off one another. I think you could do this a number of ways, but let's try binding the @Country_Code@ data element to the mark's _Y Position_ property. Since the country code is unique for each bar (as we have already filtered to only include gold medals), this make each bar move to a unique position. Try it. Drag the @Country_Code@ data element to _Y Position_ and release.
...
Now we can gild our blue bars. Modify the _Fill color_ property. You can type in a specific hex or other "css color name":https://developer.mozilla.org/en-US/docs/Web/CSS/color_value. Feel free to pick one less terrible then mine. @#e68b02@ was as gold as I wanted to get. 
...
I don't know if the visual manipulation is easier or more work then just trying out different values in the input boxes, but its nice to have options. 
...
Bind the _Y Position_ property to @Country_Code@, like you did for the rectangles. For some reason, I was having some problems with this drag-and-drop. Make sure the right circle changes color before you let go with your mouse.
...
Next, I played around with the x/y position and offset. I ended up with a @-7@ _X Position_ and a @10@ _Y Offset_. Also, I changed the color to @#777777@. Ta Da!

<div class="center">
<img class="center" src="http://vallandingham.me/images/lyra/lyra_25_small.png" alt="" style="border:0px dotted #cccccc;"/>
</div>

h3. Always Add an Axis
...
For _Type_ we are going to use @X@. For orient, we are going to use @bottom@. Now we need to associate a Scale with the axis. In the bottom left-hand corner of the screen, you should see your _Scales_ menu. Drag up your _X Scale_ and drop it in the Axis _Scale_ property. 
...
h3. Finish with a Title

Ok, my fingers are cramping up. We need to end this tutorial quick, so let's add a title.

To do this, drop another Text mark into your visualization preview. Instead of associating its text with a data element, we will just type out a title. I choose "Gold Medals by Country". You can use that too, if you want. 
...
It has some overlap with Design Density's wonderful "RAW":http://app.raw.densitydesign.org/#/ app. While RAW is more polished, I think it Lyra might ultimately provide a more powerful feature set for those interested in learning the tool. 
